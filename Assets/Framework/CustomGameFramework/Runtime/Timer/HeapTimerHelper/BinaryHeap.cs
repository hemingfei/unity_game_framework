...
namespace CustomGameFramework.Runtime
{
    ...
    public class BinaryHeap<T> where T : IBinaryHeapElement
    {
        ...
        public void Insert(T[] dataArray, eBinaryHeapBuildMode buildMode)
        {
            ...
            var totalLength = m_LastChildIndex + dataArray.Length + 1;

            if (m_Array.Length < totalLength) ResizeArray(totalLength);
            ...
            else
            {
                //数量比较大的情况下会快一些
                //方式2:先添加完，然后排序
                for (var i = 0; i < dataArray.Length; ++i) m_Array[++m_LastChildIndex] = dataArray[i];
        ...
        public void Insert(T element)
        {
            ...
            var index = ++m_LastChildIndex;

            if (index == m_Array.Length) ResizeArray();
        ...
        public T Pop()
        {
            if (m_LastChildIndex < 1) return default;

            var result = m_Array[1];
        ...
        {
            if (index > m_LastChildIndex) return;
        ...
        {
            var startChild = m_LastChildIndex >> 1;
        ...
        {
            if (index > m_LastChildIndex || index < 1) return;
        ...
        {
            if (index > m_LastChildIndex) return default;
        ...
        {
            if (newSize < 0) newSize = Math.Max(m_Array.Length + 4, (int)Math.Round(m_Array.Length * m_GrowthFactor));
            ...
            for (var i = 0; i < m_Array.Length; i++) tmp[i] = m_Array[i];
