...
namespace CustomGameFramework.Runtime
{
    ...
    {
        public readonly Dictionary<string, Texture> CacheTextures = new Dictionary<string, Texture>();

        public readonly Dictionary<string, Queue<Action<string, bool, Texture>>> GettingTextures =
            new Dictionary<string, Queue<Action<string, bool, Texture>>>();
        ...
        public void Get(string url, Action<string, bool, Texture> onLoadFinish)
        {
            ...
            else
            {
                ...
                {
                    if (!gettingTextures.Contains(onLoadFinish))
                    {
                        gettingTextures.Enqueue(onLoadFinish);
                    }
                }
                else
                {
        ...
        public void Clear()
        {
            foreach (var texture in CacheTextures.Values)
            {
                Destroy(texture);
            }
            CacheTextures.Clear();
