...
namespace CustomGameFramework.Runtime
{
    ...
    {
        public static Dictionary<UIEventType, UIEventCallBack> s_allUIEvents = new Dictionary<UIEventType, UIEventCallBack>();
        public static Dictionary<string, Dictionary<UIEventType, UIEventCallBack>> s_singleUIEvents = new Dictionary<string, Dictionary<UIEventType, UIEventCallBack>>();
        ...
        public static void FireNow(UIWindowBase ui, UIEventType uiEventType, params object[] objs)
        {
            ...
            if (s_allUIEvents.ContainsKey(uiEventType))
            {
                try
                {
                    if (s_allUIEvents[uiEventType] != null)
                    { s_allUIEvents[uiEventType](ui, objs); }
                }
                catch (Exception e)
                {
                    Log.Error("UIEvent FireNow error:" + e.ToString());
                }
            }
            ...
            switch (uiEventType)
            {
                case UIEventType.OnOpen:
                    UIManager.UIEventMgr.Fire(OpenUIFormSuccessEventArgs.EventId, OpenUIFormSuccessEventArgs.Create(ui, objs));
                    break;
                case UIEventType.OnClose:
                    UIManager.UIEventMgr.Fire(CloseUIFormCompleteEventArgs.EventId, CloseUIFormCompleteEventArgs.Create(ui.UIName, objs));
                    break;
            }
        }
