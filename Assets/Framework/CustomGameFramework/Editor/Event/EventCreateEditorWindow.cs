...
namespace CustomGameFramework.Editor.Event
{
    ...
    public class EventCreateEditorWindow : EditorWindow
    {
        ...
        [SerializeField] private List<EventArgs> _eventArgs = new();
        ...
        private void OnGUI()
        {
            ...
            if (_generatedFilePath.Length > 7)
                ...
                EditorGUILayout.LabelField("文件路径",
                    Application.dataPath + "/" + _generatedFilePath.Replace("Assets/", string.Empty));
        ...
        {
            var window = (EventCreateEditorWindow)GetWindow(typeof(EventCreateEditorWindow));
        ...
        private void OnOpen()
        {
            ...
            if (string.IsNullOrEmpty(_generatedFilePath)) _generatedFilePath = _defaultGeneratedFilePath;

            _eventNamespace = EditorPrefs.GetString(EditorPrefsEventNameSpace);
            if (string.IsNullOrEmpty(_eventNamespace)) _eventNamespace = _defaultNamespace;
        }
        ...
        private void EventCreate()
        {
            ...
            if (_generatedFilePath.Length > 7)
                ...
                scriptPath = Application.dataPath + "/" + _generatedFilePath.Replace("Assets/", string.Empty) + "/" +
                             _eventName + "EventArgs.cs";
            ...
            //EventParams
            s = "";
            for (var i = 0; i < _eventArgs.Count; i++)
            {
                var _argname = _eventArgs[i].ArgName;
                _argname = _argname.Substring(0, 1).ToLower() + _argname.Substring(1);
                var _argDes = _eventArgs[i].ArgDes;
            ...
            var dirPath = scriptPath.Substring(0, scriptPath.LastIndexOf("/"));
            if (!Directory.Exists(dirPath)) Directory.CreateDirectory(dirPath);

            // write
            using (var file = new FileStream(scriptPath, FileMode.CreateNew))
            {
                using (var fileW = new StreamWriter(file, Encoding.UTF8))
                {
